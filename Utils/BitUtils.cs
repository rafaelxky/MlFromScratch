public static class BitUtils
{
    ...
    public static byte[,] PackMatrix(int[,] matrix)
    {
        ...
        int byteCount = (colCount + 3) / 4;

        byte[,] output = new byte[rowCount, byteCount];

        for (int neuronId = 0; neuronId < rowCount; neuronId++)
        {
            for (int i = 0; i < byteCount; i++)
            {
                int v0 = i * 4 + 0 < colCount ? matrix[neuronId, i * 4 + 0] : 0;
                int v1 = i * 4 + 1 < colCount ? matrix[neuronId, i * 4 + 1] : 0;
                int v2 = i * 4 + 2 < colCount ? matrix[neuronId, i * 4 + 2] : 0;
                int v3 = i * 4 + 3 < colCount ? matrix[neuronId, i * 4 + 3] : 0;

                output[neuronId, i] = Pack(v0, v1, v2, v3);
            }
        }
    ...
    public static double PackedDotProduct(byte[,] layer, int neuronId, double[] values, int lenght)
    {
        ...
        for (int i = 0; i < lenght; i++)
        {
            ...
            int a = GetPair(layer[neuronId, byteIndex], pairIndex);
